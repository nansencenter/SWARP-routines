...
WORKDIR=/work/bergh/BS1a0.045/expt_01.6/
...
## DONT NEED THIS - RUN MODEL IN MORNING AT DAY1
##julday=`expr $julday`
#julday=`expr $julday + 1`
###### RESTART CHANGE TEMPOR. ###
##  julday=`expr $julday - 1`
#################################
julday_end=`expr $julday + 2`
echo "julday = $julday"
echo ""
...
echo " * Simulation will run from $year day $day to day $day2_hycom"
echo ""
...
echo ""
${MAINDIR}/makeinfile4waveforecast.sh BS1 $year $day $day1_hycom $day2_hycom
...
echo " * Integrating 48 hours forecast. Will process results when run completed"
echo ""
cd ${WORKDIR}
#/opt/torque/torque-2.3.4-snap.200809221601/bin/qsub pbsjob_BS1_RT.sh
/opt/torque/2.5.11/bin/qsub pbsjob_BS1_WIM_RT.sh
...
do
  ...
  then
    echo " ***********************************************************************************"
    echo " ******* PBS JOB NOT COMPLETED AFTER 6 HOURS - EXIT barents_forecast_WIM.sh ********"
    echo " ***********************************************************************************"
    exit 
  fi
  count=`expr $count + 1`
...
${MAINDIR}/barents_postprocessing_WIM.sh $julday
echo " * Launching barents_forecast_postprocessing_WIM.sh $julday in jobname: RT-WIM-BS1 "
echo ""
