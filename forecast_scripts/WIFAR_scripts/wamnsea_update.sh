...
done # end of download while loop
...
if [ -f wam_nsea.an.${thedate}.nc ]
 then
  /work/apps/cdo/1.5.4-cray/bin/cdo cat wam_nsea.an.${thedate}.nc wam_nsea_${year}.nc
else
...
if [ $year -eq $year1d -a $year -eq $year2d ]
 then
  echo "Same year all 3 days"
  rm wam_nsea_fc_${year}.nc
  /work/apps/cdo/1.5.4-cray/bin/cdo copy wam_nsea_${year}.nc wam_nsea.fc.${thedate}.nc wam_nsea_fc_${year}.nc
elif [ $year -eq $year1d -a $yearplus1 -eq $year2d ]
 then
  echo "Same year today and today +1 - new year today +2"
  rm wam_nsea_fc_${year}.nc
  /work/apps/cdo/1.5.4-cray/bin/cdo copy wam_nsea_${year}.nc wam_nsea.fc.${thedate}.nc wam_nsea_fc_${year}.nc
  ...
  /work/apps/cdo/1.5.4-cray/bin/cdo cat wam_nsea_fc_split_${yearplus1}.nc wam_nsea_fc_${yearplus1}.nc
  rm wam_nsea_fc_split_*
elif [ $yearplus1 -eq $year1d -a  $yearplus1 -eq $year2d ]
 then
  echo "New year today +1 and today +2"
  rm wam_nsea_fc_${yearplus1}.nc
  /work/apps/cdo/1.5.4-cray/bin/cdo copy wam_nsea.fc.${thedate}.nc wam_nsea_fc_${yearplus1}.nc
  echo "ERROR when updating WAMNSEA data - year not correct"
fi
echo ""
