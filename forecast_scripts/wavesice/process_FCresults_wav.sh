...
if [ -f $datelist ]
then
   $SWARP_ROUTINES/forecast_scripts/wavesice/gather_FCresults.sh  $(cat $datelist | sed '1!d') $(cat $datelist | sed '2!d') #$1 $2 
   ...
   $SWARP_ROUTINES/forecast_scripts/wavesice/collect_FCresults.sh $(cat $datelist | sed '1!d') $(cat $datelist | sed '2!d') #$1 $2
