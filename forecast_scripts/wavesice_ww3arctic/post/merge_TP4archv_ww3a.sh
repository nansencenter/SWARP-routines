...
# wave forcing info
ncatted -O -h -a wave_forcing,global,c,c,"Wavewatch 3 Arctic (Ifremer)"       $ofil
ncatted -O -h -a wave_forcing_reference,global,c,c,"ftp://ftp.ifremer.fr/ifremer/cersat/products/gridded/wavewatch3/iowaga/HINDCAST/ARCTIC"                 $ofil
#
...
then
   efil=swarp_tmp.txt
   echo Warning: merge_TP4archv_ww3a.sh                           >  $efil
   echo Wrong number of records in $ofil                          >> $efil
   echo "($Nfiles -  should be $Ncorrect)"                        >> $efil
   mail -s "WARNING: waves-ice (WW3) final product faulty" $email <  $efil
   rm $efil
else
   efil=swarp_tmp.txt
