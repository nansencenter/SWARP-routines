...
then
   ...
   Fdate=`date --date="$cdate -1days" +%Y%m%d`
   Fyear=`date --date="$Fdate" +%Y`
   fc_fil=$ww3_arctic/$Fyear/forecast/SWARP_WW3_ARCTIC-12K_$Fdate.fc.nc
   if [ ! -f $fc_fil ]
   then
      if [ $print_info -eq 1 ]
      then
         echo "WAM input FC is not yet present - stopping"
         echo $fc_fil
         date
      fi
      exit
