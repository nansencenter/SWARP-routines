#!/bin/bash
# This script will: 
# 1) Create today's hindi_datelist
...
echo $(date +%Y%m%d)       >> $hindi_datelist
echo $(date +%Y-%m-%d)     >> $hindi_datelist
echo $(date +%Y)           >> $hindi_datelist
echo $(date +%m)           >> $hindi_datelist
echo $(date +%d)           >> $hindi_datelist
...
rundir=/work/timill/RealTime_Models/results/TP4a0.12/ice_only/work/$(cat $hindi_datelist | sed '1!d') # where the last_restart.txt will end up
cd $rundir
mkdir -p ./info
...
year_today=$(cat $hindi_datelist | sed '3!d')
