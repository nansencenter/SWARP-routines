...
# Data reader
class reader:
    def __init__(self,dadate,bmm=None,bmo=None):
    ...
    def _read_osi_(self,dadate,basemap=None):
        day = self.day
        month = self.month
        year = self.year
    ...
    def _read_mdl_ice_only(self,data_date,basemap=None):
    ...
    def _read_mdl_waves_ice(self,data_date,basemap=None):
...
def HS_save(data,data2,region):
   ndata = mask_region(data,region)
   ndata2 = mask_region(data2,region)
   MWD_avg = np.nanmean(ndata2)
   HS_max = np.nanmax(ndata)
...
# optional basemap (mdl,osi) for X,Y
data = reader(dadate)
print('DONE')
print('')

print('Getting the var data...')
